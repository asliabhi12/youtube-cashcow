...
from app.models.job import JobLogEntry, JobLogLevel
...
class JobLogHub:
    ...
    ) -> tuple[list[JobLogEntry], "asyncio.Queue[object]"]:
        """Atomically snapshot history and subscribe to future entries.

        Returns the history at subscription time and a queue that will receive
        every entry appended afterwards, followed by the close sentinel when the
        job terminates. If the job has already closed, the sentinel is enqueued
        immediately so the caller stops once it has drained the snapshot.
        """
